...
import eu.scape_project.pw.planning.criteria.bean.data.PotentialToRangeMaxData;
...
public class KBrowser implements Serializable {
	 ...
	 */
	private SortOrder[] cifIfSortOrder = { SortOrder.unsorted,
			SortOrder.unsorted, SortOrder.descending, SortOrder.unsorted,
	 ...
	 */
	private SortOrder[] cifIfSortOrderCompact = { SortOrder.unsorted,
			SortOrder.unsorted, SortOrder.descending, SortOrder.unsorted,
			SortOrder.unsorted };
	...
	public void initBean() {
		...
		filteredMeasurableProperties = new ArrayList<MeasurableProperty>(
				allMeasurableProperties);
	...
	public void initData() {
		...
		importanceAnalysis = new ImportanceAnalysis(allMeasurableProperties,
				planLeaves, nrRelevantPlans);
	 ...
	 *         once.
	 */
	private Collection<MeasurableProperty> identifyUsedMeasurableProperties(
			List<VPlanLeaf> planLeaves) {
		...
		for (VPlanLeaf l : planLeaves) {
			if (l.getCriterion() != null) {
				if (l.getCriterion().getProperty() != null) {
					mappedProperties.put(l.getCriterion().getProperty()
							.getPropertyId(), l.getCriterion().getProperty());
				}
			}
		}
	...
	public void setSelectedMeasurableProperty(
			MeasurableProperty selectedMeasurableProperty) {
	...
	public void setFilteredMeasurableProperties(
			Collection<MeasurableProperty> filteredMeasurableProperties) {
	...
	public void setSelectedMeasurablePropertyString(
			String selectedMeasurablePropertyString) {
		...
		if (selectedMeasurablePropertyString == null) {
			...
			selectedMeasurableProperty = measurablePropertiesMap
					.get(selectedMeasurablePropertyString);
		}
	}
	...
	public void selectProperty() {
		...
		if (selectedMeasurableProperty == null) {
			...
			log.info("Property selected: "
					+ selectedMeasurableProperty.getName());
		}
	...
	public void filterMeasurableProperties() {
		...
		for (MeasurableProperty p : (showOnlyUsedProperties ? usedMeasurableProperties
				: allMeasurableProperties)) {
		...
		if (selectedMeasurableProperty != null) {
			for (MeasurableProperty mp : filteredMeasurableProperties) {
				if (mp.getPropertyId().equals(
						selectedMeasurableProperty.getPropertyId())) {
	...
	public void updateMetrics() {
		...
		if (selectedMeasurableProperty == null) {
			...
			constructMetricsWithMap(selectedMeasurableProperty
					.getPossibleMetrics());
		}
	...
	private void checkInputAndCalculateSelectiveStatistics() {
		...
		if (selectedCategory != null) {
			...
			nrPlanLeavesInCategory = calculator
					.getNrPlanLeavesInCategory(selectedCategory);
		} else {
		...
		if (selectedMeasurableProperty != null) {
			...
			nrPlanLeavesUsingProperty = calculator
					.getNrPlanLeavesUsingProperty(selectedMeasurableProperty);
		...
		if ((selectedMeasurableProperty != null && selectedMetric != null)
				|| (selectedMeasurableProperty != null && selectedMeasurableProperty
						.getScale() != null)) {
			isMeasurableCriterionSelected = true;
			calculator.setCriterion(criteriaManager.getCriterion(
					selectedMeasurableProperty, selectedMetric));
			...
			nrCPLMeasurementsObtained = calculator
					.getNrCPLMeasurementsObtained();
			nrCPLEvaluations = calculator.getCPLEvaluations().size();
			transformerTable = calculator.getCPSTransformerTable();
			log.debug("TRANSFORMERTABLE-TargetValues: "
					+ transformerTable.getTargetValues().size());
			...
			if ((scale instanceof FloatRangeScale)
					|| (scale instanceof FloatScale)
					|| (scale instanceof IntegerScale)
					|| (scale instanceof IntRangeScale)
					|| (scale instanceof PositiveFloatScale)
					|| (scale instanceof PositiveIntegerScale)) {
				hasCriterionNumericScale = true;
				hasCriterionOrdinalScale = false;
				cplNumericMeasurementsMin = calculator
						.getCPLNumericMeasurementsMin();
				cplNumericMeasurementsMax = calculator
						.getCPLNumericMeasurementsMax();
				cplNumericMeasurementsAvg = calculator
						.getCPLNumericMeasurementsAvg();
			}
	...
	public void setIsMeasurableCriterionSelected(
			Boolean isMeasurableCriterionSelected) {
	...
	public void setCplOrdinalMeasurements(
			Map<String, Integer> cplOrdinalMeasurements) {
	...
	public List<Map.Entry<String, Integer>> getCplOrdinalMeasurementsList() {
		return new ArrayList<Map.Entry<String, Integer>>(
				cplOrdinalMeasurements.entrySet());
	}
	...
	public void setUsedMeasurablePropertiesCount(
			int usedMeasurablePropertiesCount) {
	...
	public void setCplAvgPotentialOutputRangeOccurrenceBased(
			double cplAvgPotentialOutputRangeOccurrenceBased) {
	...
	public void setCplAvgActualOutputRangeOccurrenceBased(
			double cplAvgActualOutputRangeOccurrenceBased) {
	...
	public void sortCifByColumnCompact(int column) {
		log.debug("Sorting compact Criterion Impact Factors by IF"
				+ Integer.toString(column));
	...
	private void calculatePotentialToRangeScale() {
		...
		for (ImportanceAnalysisProperty property : importanceAnalysis
				.getTableRows()) {
		...
		potentialToRangeScale = new BigDecimal(scaleValue, new MathContext(2))
				.doubleValue();
	}
}
