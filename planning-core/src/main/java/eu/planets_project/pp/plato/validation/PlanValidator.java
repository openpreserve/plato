...
public class PlanValidator implements Serializable {
	...
	@Inject TreeValidator treeValidator;
	 ...
	 * - Does <p>not</p> validate the preceding plan states.
	 * 
	 * @param plan
	 * @param state
	 * @return
	...
	private boolean isDecisionChosenSatisfied(Plan plan,
		...
		// FIXME add validation of required fields, - javax - validation constraints 

		return result;
	}

	private boolean isExperimentDefinedSatisfied(Plan plan,
			List<ValidationError> errors) {
	...
	private boolean isExperimentPerformedSatisfied(Plan plan,
			List<ValidationError> errors) {
		return true;
	}

	private boolean isResultsCapturedSatisfied(Plan plan,
			List<ValidationError> errors) {
        // FIXME add validation of required fields, - javax - validation constraints

		// we do not want to define a specific NodeValidator, therefore we create an anonymous class
        // we have to make this plan available as final variable, so the anonymous class can access it
		final Plan thisPlan = plan;

		boolean result = treeValidator.validate(plan.getTree().getRoot(), 
			new INodeValidator() {
				private List<Alternative> consideredAlternatives = thisPlan.getAlternativesDefinition().getConsideredAlternatives();
	...
	private boolean isExecutablePlanCreatedSatisfied(Plan plan,
			List<ValidationError> errors) {
		// no validation at this step
		return true;
	}

	private boolean isPlanDefinedSatisfied(Plan plan,
			List<ValidationError> errors) {
		// no validation at this step
		return true;
	}

	private boolean isPlanValidatedSatisfied(Plan plan,
			List<ValidationError> errors) {
		// no validation at this step
		return true;
	}
	...
	private boolean isAlternativesDefinedSatisfied(Plan plan, List<ValidationError> errors) {
		...
		if (plan.getAlternativesDefinition().getAlternatives().size() <= 0) {
			ValidationError error = new ValidationError("At least one alternative must be added to proceed with the workflow.");
			errors.add(error);
			return false;
		}
	...
	private boolean isRecordsChosenSatisfied(Plan plan, List<ValidationError> errors) {
        ...
        for (SampleObject sample: plan.getSampleRecordsDefinition().getRecords()) {
            if (names.contains(sample.getShortName())) {
               result = false;
               errors.add(new ValidationError("There are two samples with the same short name '"+sample.getShortName()+"'. Please provide unique names."));
            }
	...
	private boolean isFteRequirementsDefinedSatisfied(Plan plan, List<ValidationError> errors) {
		// FteRequirementsDefined state is satisfied when the corresponding full-wf steps
