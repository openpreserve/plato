 ...
 *  - and also takes care of caching bytestreams.
 * Note:
 *  - It does NOT take any measures to prevent concurrency issues,ensure thread savety.
 *  But this is ok, because : 
 *  - A plan cannot be loaded and accessed more than once at a time, the ByteStreamManager shares the same
 *    (conversation) scope. 
 *  - Beside different planning-conversations we do not read/store bytestreams with multiple threads.     
 * 
...
public class ByteStreamManager implements Serializable, IByteStreamManager {
    ...
    /**
     * Stores the given bytestream to a tempfile and keeps the handle for later use.
     *   
     * @param pid
     * @param bitstream
     */
