...
//	        setPlanlist("public preservation plans");
	 ...
	 * @param propertyId the plan's PROPERTIES id!
	 */
	public Plan load(int propertyId) throws PlanningException {
	    ...
	    if (num < 1) {
	    	throw new PlanningException( "In the meantime the plan has been loaded by an other user. Please choose another plan.");
	    }
	    Object result = em.createQuery(
	            "select p.id from Plan p where p.planProperties.id = " + propertyId).getSingleResult();
