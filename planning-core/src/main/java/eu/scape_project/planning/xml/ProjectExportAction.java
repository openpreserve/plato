...
public class ProjectExportAction implements Serializable {
     ...
     * 
     * @param fromID from-ID in table PlanProperties, which is used to filter PlanProperties
     * @param toID to-ID in table PlanProperties, which is used to filter PlanProperties
     * @return True if export was successful, false otherwise.
     */
    public boolean exportSomeProjectsToZip(Integer fromID, Integer toID){
        List<PlanProperties> ppList = em.createQuery(
                "select p.planProperties from Plan p where " +
                 " p.planProperties.id >= "+fromID+" and p.planProperties.id <= "+toID +
...
//                    writer.close();
    ...
    /** 
     * new helper method that was refactored from {@link #exportAllProjectsToZip()}
     * It takes a list of {@link PlanProperties} and exports it to a zip file.
     * @param ppList {@link PlanProperties} for plans to export
     * 
     * @return True if export was successful, false otherwise.
     */
