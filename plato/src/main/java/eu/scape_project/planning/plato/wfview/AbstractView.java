...
public abstract class AbstractView implements Serializable{
	...
	/**
	 * Initializes the view with the given plan.
	 * This includes initializing the corresponding BL
	 *  
	 * @param plan
	 */
     ...
     * Derived views may use this also to undo changes which do not concern the plan,
     * like deleting created files... 
     */
    public void discard() {
    	try {
			getWfStep().discard();
	 ...
	 * - this includes validating if the plan has progressed enough
	 * - if there are problems, they are displayed as FacesMessages
	 * 
	 * @return "success", if the plan state could be advanced, <code>null</code> otherwise  
	 */
    ...
    /**
     * Does the actual proceed, for internal use only.
     * Any occurring errors should be added to the provided list.
     *  
     * @param errors
	...
	/**
	 * Touches the given object.
	 * Used for a4j:ajax listener, which can not cope with complex method el expressions
	 * 
	 * @param object
	 */
