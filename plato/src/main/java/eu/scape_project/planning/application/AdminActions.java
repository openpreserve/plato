...
public class AdminActions implements Serializable {
	 ...
	 * 
	 * @param password Password to check for correctness.
	...
	/**
	 * Exports all plans with planproperty-ids between fromPlanPropertiesId and toPlanProperitesId into a single xml file.
	 * 
	 * @param fromPlanPropertiesId Start of the id range to export.
	 * @param toPlanProperitesId End of the id range to export.
	 * @return True if export was successful, false otherwise.
	 */
	...
	/**
	 * Method responsible for retrieving the path where the last project export was put into.
	 * 
	 * @return Path where the last project export was put into, or null if no project export was done in this session yet.
	 */
	...
	public void deleteAllPlans() {
        ...
        for (Plan p : planList) {
            ...
            // this part does not work - but it is not needed, so it is commented-out.
	 ...
	 * 
	 * @param planPropertiesId PlanProperties-id of the plan to unlock.
	 ...
	 * 
	 * @param planPropertiesId PlanProperties-id of the plan to clone.
	 * @return True if cloning was successful, false otherwise.
	 */
	public boolean clonePlan(Integer planPropertiesId) {
		List<Plan> planList = em.createQuery("select p from Plan p where p.planProperties.id = " + planPropertiesId).getResultList();
        ...
        try {
            ...
            List<Plan> plans = projectImporter.importProjects(new FileInputStream(tempFile));
	 ...
	 * 
	 * @param planPropertiesId PlanPropertiesId of the plan to delete.
	 * @return True if deletion was successful, false otherwise.
	 */
	public boolean deletePlan(Integer planPropertiesId) {
        List<Plan> projectList = em.createQuery("select p from Plan p where p.planProperties.id = " + planPropertiesId).getResultList();
	 ...
	 * 
	 * @param mb Memory to munch (in MB).
	 */
	 ...
	 * 
	 * @param file File which contains plan in xml-format.
	 * @param changeUser Indicates if the imported plan should be assigned to the current user (thus be imported for only this user) 
	 * @return Number of plans imported successfully.
	 */
	public int importPlansFromFile(byte[] fileData, boolean changeUser) {
		...
		// start import
		try {
			plansToImport = projectImporter.importProjects(new ByteArrayInputStream(fileData));
	...
	/**
	 * Method responsible for importing plans via their given xml representation.
	 * 
	 * @param xml Xml representation of the plans.
	 * @return Number of plans imported successfully.
	 */
	public int importPlansFromXml(String xml) {
		...
		try {
			plansToImport = projectImporter.importProjects(new ByteArrayInputStream(xml.getBytes("UTF-8")));
		}
		catch (Exception e) {
	...
	/**
	 * Method responsible for cleaning-up/removing loose Plan Values for a given Plan.
	 * 
	 * @param pid PlanProperties id of the Plan to operate on.
	...
	/**
	 * Method responsible for computing an passcode (SHA encoded/encrypted password) out of the given password.
	 * 
	 * @param password Password to compute the encoded passcode for.
