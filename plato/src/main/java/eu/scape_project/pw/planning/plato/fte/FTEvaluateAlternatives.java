...
import eu.scape_project.pw.planning.plato.wf.EvaluateExperiments;
...
public class FTEvaluateAlternatives extends AbstractWorkflowStep {
	...
	public void init(Plan p) {
        ...
        boolean valueMapProperlyInitialized = 
            plan.getTree().getRoot().isValueMapProperlyInitialized(plan.getAlternativesDefinition().getConsideredAlternatives(), plan.getSampleRecordsDefinition().getRecords().size());
	...
	protected boolean mayProceed(List<ValidationError> errors) {
	   ...
	   // Rest of the validation is the same as in {@link EvaluateExperimentsAction.validate}
	...
	/**
	 * Removes the given alternative from the plan's list of alternatives
	 * - also takes care of values of this alternative
	 *  
	 * @param alternative
	 */
    public void removeAlternative(Alternative alternative) {
        ...
        // important: we also have to remove the values which might have been added here...
    ...
    private void addServices() {
        ...
        SampleObject sample = plan.getSampleRecordsDefinition()
				...
				if (reg.getShortname().contains("MiniMEE")) {
				    ...
				    for (PreservationActionDefinition definition : actions) {
				        Alternative a = Alternative.createAlternative(
				                plan.getAlternativesDefinition().createUniqueName(definition.getShortname()),
				                definition);
				        // and add it to the preservation planning project
				        plan.getAlternativesDefinition()
				                .addAlternative(a);
				    }
				}
